...
greedy = np.zeros((1, num_steps), dtype = int)
get_greedy = False
# mutate_threshold = .05
...
while True:
    ...
    if j == inp.shape[0]:
        ...
        # print(scores)
        logit_parent = logit(parent)
        index_max = int(np.where(scores == np.max(scores))[1][0])
        best_kid = inp[index_max, :]

        for m in range(inp.shape[1]):
        ...
        parent = sigmoid(logit_parent)

        for k in range(inp.shape[1]):

            greedy[0, k] = int(np.where(parent[:, k] == np.max(parent[:, k]))[0][0])

            for l in range(inp.shape[0]):

                if (np.random.rand() * (parent[0, k] + parent[1, k] + parent[2, k]))  <= parent[0, k]:

                    inp[l, k] = 0

                elif (np.random.rand() * (parent[0, k] + parent[1, k] + parent[2, k])) <= (parent[0, k] + parent[1, k]):
